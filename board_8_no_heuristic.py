...
class Board_8_no_heuristic:
    ...
    def up(self):
        if self.zero > 2:
            return Board_8_no_heuristic(self.swap(self.zero, self.zero - 3), self, 'Up', self.depth + 1)
        else:
            return None

    def down(self):
        if self.zero < 6:
            return Board_8_no_heuristic(self.swap(self.zero, self.zero + 3), self, 'Down', self.depth + 1)
        else:
            return None

    def left(self):
        if self.zero % 3 != 0:
            return Board_8_no_heuristic(self.swap(self.zero, self.zero - 1), self, 'Left', self.depth + 1)
        else:
            return None

    def right(self):
        if (self.zero + 1) % 3 != 0:
            return Board_8_no_heuristic(self.swap(self.zero, self.zero + 1), self, 'Right', self.depth + 1)
        else:
            return None

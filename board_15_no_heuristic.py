...
class Board_15_no_heuristic:
    ...
    def up(self):
        if self.zero > 3:
            return Board_15_no_heuristic(self.swap(self.zero, self.zero - 4), self, 'Up', self.depth + 1)
        else:
            return None

    def down(self):
        if self.zero < 12:
            return Board_15_no_heuristic(self.swap(self.zero, self.zero + 4), self, 'Down', self.depth + 1)
        else:
            return None

    def left(self):
        if self.zero % 4 != 0:
            return Board_15_no_heuristic(self.swap(self.zero, self.zero - 1), self, 'Left', self.depth + 1)
        else:
            return None

    def right(self):
        if (self.zero + 1) % 4 != 0:
            return Board_15_no_heuristic(self.swap(self.zero, self.zero + 1), self, 'Right', self.depth + 1)
        else:
            return None

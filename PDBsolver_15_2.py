...
class Solver():
    ...
    def load_pattern_database(self):
        file = open("15_D_database", "rb")
        self.__pattern_database_A = pickle.load(file)
        file.close()

        file = open("15_E_database", "rb")
    ...
    def get_heuristic_value(self, cur_state):
        A = [a if a in [1, 2, 3, 4, 5, 6, 7, 8] else 0 for a in cur_state]
        B = [b if b in [9, 10, 11, 12, 13, 14, 15] else 0 for b in cur_state]
        ret = 0
...
if __name__ == "__main__":
    ...
    filename = "results/" + "PDB_15_sym.csv"
    result_file = open(filename, "w", buffering=1)
    result_file.write("{}\n".format("15-puzzle using A* with PDB(symmetric look-up)"))
